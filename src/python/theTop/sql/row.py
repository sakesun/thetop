class Row(object):
    ...
    def update(self, **settings):
        ...
        for k,v in settings.items():
            if isinstance(v, expr.Expression):
    ...
    def evaluator(self, **params):
        return expr.Evaluator(items=self.dict(), params=params) \
               if self.table is None \
               else self.table.create_evaluator(items = self.dict(), params=params)
    def evaluate(self, expression, params={}): return expr.build(expression).express(self.evaluator(**params))
